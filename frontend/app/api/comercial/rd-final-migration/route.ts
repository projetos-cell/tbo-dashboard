...
async function rdFetchDealActivities(
  dealId: string,
  token: string,
): Promise<RdActivity[]> {
  try {
...
async function rdFetchDealNotes(
  dealId: string,
  token: string,
): Promise<RdNote[]> {
  try {
...
async function rdFetchDealTasks(
  dealId: string,
  token: string,
): Promise<RdTask[]> {
  try {
...
function inferWonLost(name?: string): "won" | "lost" | null {
  if (!name) return null;
...
function mapStage(rdName: string | undefined, isWon: boolean, isClosed: boolean): string {
