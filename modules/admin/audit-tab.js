// ============================================================================
// TBO OS — Admin Portal: Tab Auditoria
// Logs de auditoria do Supabase, filtros, paginacao, export CSV
// Dados carregados do Supabase, zero hardcode
// ============================================================================

const TBO_ADMIN_AUDIT_TAB = {

  _logs: [],
  _filters: { dateFrom: null, dateTo: null, userId: null, action: null, entity: null },
  _page: 0,
  _pageSize: 50,
  _total: 0,
  _portal: null,
      ...
      <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            ...
            <button class="btn btn-ghost btn-sm" id="apExportAudit"><i data-lucide="download" style="width:14px;height:14px;"></i> CSV</button>
            <button class="btn btn-ghost btn-sm" id="apRefreshAudit"><i data-lucide="refresh-cw" style="width:14px;height:14px;"></i></button>
          </div>
        </div>

        <!-- Filtros -->
        <div class="ap-filter-bar">
          <input type="date" class="ap-filter-input" id="apFilterDateFrom" value="${this._filters.dateFrom || ''}" placeholder="De">
          <input type="date" class="ap-filter-input" id="apFilterDateTo" value="${this._filters.dateTo || ''}" placeholder="Ate">
          <select class="ap-filter-input" id="apFilterAction">
          ...
          <select class="ap-filter-input" id="apFilterEntity">
          ...
          <button class="btn btn-ghost btn-sm" id="apApplyFilter">Filtrar</button>
        </div>
        ...
        ${this._total > this._pageSize ? `
          <div class="ap-pagination">
            <button class="btn btn-ghost btn-sm" id="apPrevPage" ${this._page === 0 ? 'disabled' : ''}>← Anterior</button>
            <span style="font-size:0.72rem;color:var(--text-muted);">Pagina ${this._page + 1} de ${totalPages}</span>
            <button class="btn btn-ghost btn-sm" id="apNextPage" ${this._page >= totalPages - 1 ? 'disabled' : ''}>Proxima →</button>
          </div>
        ` : ''}
      </div>
    `;
  ...
  bind() {
    document.getElementById('apApplyFilter')?.addEventListener('click', () => {
      this._filters.dateFrom = document.getElementById('apFilterDateFrom')?.value || null;
      this._filters.dateTo = document.getElementById('apFilterDateTo')?.value || null;
      this._filters.action = document.getElementById('apFilterAction')?.value || null;
      this._filters.entity = document.getElementById('apFilterEntity')?.value || null;
      this._page = 0;
      this.loadLogs();
    });

    document.getElementById('apRefreshAudit')?.addEventListener('click', () => this.loadLogs());
    document.getElementById('apExportAudit')?.addEventListener('click', () => this._exportCSV());
    document.getElementById('apPrevPage')?.addEventListener('click', () => { this._page--; this.loadLogs(); });
    document.getElementById('apNextPage')?.addEventListener('click', () => { this._page++; this.loadLogs(); });
  },
  ...
  async loadLogs() {
    ...
    try {
      ...
      console.warn('[Admin Audit] Erro ao carregar logs:', e.message);
    }
  },
