...
function TeamOKRsContent() {
  ...
  const [objDialog, setObjDialog] = useState<{ open: boolean; objective?: ObjectiveRow | null }>({
    open: false,
  });
  ...
  const [cycleDialog, setCycleDialog] = useState<{ open: boolean; cycle?: CycleRow | null }>({
    open: false,
  });
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast({
        title:
          deleteTarget.type === "objective" ? "Objetivo excluído" : "Key Result excluído",
      });
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">Objetivos e resultados-chave por equipe</p>
        </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            <div className="rounded-full bg-gray-100 p-4 mb-4">
              <IconTarget className="h-10 w-10 text-gray-400" />
            </div>
            <h2 className="text-lg font-semibold mb-2">Nenhum OKR de equipe neste ciclo</h2>
            <p className="text-gray-500 text-sm max-w-md mb-6">
              OKRs de equipe conectam os objetivos estratégicos da empresa com as entregas dos
              times. Crie o primeiro para começar.
            </p>
            <RequireRole minRole="lider">
              <Button onClick={() => setObjDialog({ open: true })}>
                <IconPlus className="h-4 w-4 mr-1" />
                Criar Objetivo de Equipe
              </Button>
            </RequireRole>
          </CardContent>
        </Card>
        ...
        <div className="space-y-3">
          {(objectives ?? []).map((obj) => (
            <OkrObjectiveCard
              ...
              onDelete={(o) =>
                setDeleteTarget({ type: "objective", id: o.id, title: o.title })
              }
            ...
            >
              <OkrKeyResultList
                ...
                onEditKr={(kr) =>
                  setKrDialog({ open: true, objectiveId: obj.id, keyResult: kr })
                }
                onDeleteKr={(kr) =>
                  setDeleteTarget({ type: "kr", id: kr.id, title: kr.title })
                }
