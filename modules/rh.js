...
const TBO_RH = {
  ...
  _team: [
    // Diretoria
    { id: 'ruy',     nome: 'Ruy Lima',           cargo: 'Diretor Comercial',       area: 'Vendas, Operacao',      bu: 'Vendas',       nivel: 'Senior III', lider: null },
    { id: 'marco',   nome: 'Marco Andolfato',    cargo: 'Diretor Operacoes',       area: 'Operacao',              bu: '',              nivel: 'Senior III', lider: null },
    // Coordenacao / POs — report Marco
    { id: 'carol',   nome: 'Carol',              cargo: 'Coord. Atendimento',      area: 'Pos Vendas, Operacao',  bu: '',              nivel: 'Pleno II',   lider: 'marco' },
    { id: 'nelson',  nome: 'Nelson',             cargo: 'PO Branding',             area: 'BU Branding',           bu: 'Branding',      nivel: 'Pleno III',  lider: 'marco' },
    { id: 'nath',    nome: 'Nathalia',           cargo: 'PO Digital 3D',           area: 'BU Digital 3D',         bu: 'Digital 3D',    nivel: 'Senior I',   lider: 'marco' },
    { id: 'rafa',    nome: 'Rafa',               cargo: 'PO Marketing',            area: 'BU Marketing',          bu: 'Marketing',     nivel: 'Pleno II',   lider: 'marco' },
    // Report Ruy
    { id: 'gustavo', nome: 'Gustavo',            cargo: 'Comercial',               area: 'Vendas',                bu: 'Vendas',        nivel: 'Pleno I',    lider: 'ruy' },
    // Time Branding — report Nelson
    { id: 'celso',   nome: 'Celso',              cargo: 'Designer',                area: 'BU Branding',           bu: 'Branding',      nivel: 'Pleno I',    lider: 'nelson' },
    { id: 'erick',   nome: 'Erick',              cargo: 'Designer',                area: 'BU Branding',           bu: 'Branding',      nivel: 'Jr. III',    lider: 'nelson' },
    // Time Digital 3D — report Nath
    { id: 'dann',    nome: 'Danniel',            cargo: 'Lider Tecnico 3D',        area: 'BU Digital 3D',         bu: 'Digital 3D',    nivel: 'Senior I',   lider: 'nath' },
    { id: 'duda',    nome: 'Duda',               cargo: 'Artista 3D',              area: 'BU Digital 3D',         bu: 'Digital 3D',    nivel: 'Jr. III',    lider: 'dann' },
    { id: 'tiago',   nome: 'Tiago M.',           cargo: 'Artista 3D',              area: 'BU Digital 3D',         bu: 'Digital 3D',    nivel: 'Pleno I',    lider: 'dann' },
    { id: 'mari',    nome: 'Mariane',            cargo: 'Artista 3D',              area: 'BU Digital 3D',         bu: 'Digital 3D',    nivel: 'Jr. II',     lider: 'dann' },
    // Time Marketing — report Rafa
    { id: 'lucca',   nome: 'Lucca',              cargo: 'Analista de Marketing',   area: 'BU Marketing',          bu: 'Marketing',     nivel: 'Jr. III',    lider: 'rafa' },
    // Terceirizados (nao participam de avaliacao)
    { id: 'financaazul', nome: 'Financa Azul',   cargo: 'Financeiro (terc.)',      area: 'Financeiro',            bu: '',              nivel: '',           lider: 'ruy', terceirizado: true }
  ],
      ...
      <div class="rh-module">
        ...
        <div class="tab-content active" id="tab-rh-avaliacao">
          ...
          <div class="tab-bar tab-bar--sub" id="rhAvalSubtabs" style="margin-bottom:16px;">
            ...
            <button class="tab tab--sub" data-subtab="aval-avaliar">Avaliar</button>
          ...
          <!-- Subtab: Avaliar -->
          <div class="subtab-content" id="subtab-aval-avaliar">
            ${this._renderAvaliarForm(activeCycle, reviews)}
          </div>
    ...
    return `
      ...
      <div class="grid-3" style="gap:16px;margin-bottom:20px;align-items:start;">
        ...
        <div class="card" style="padding:20px;text-align:center;">
          ...
          <div style="font-weight:700;font-size:1rem;">${TBO_FORMATTER.escapeHtml(person.nome)}</div>
          <div style="font-size:0.78rem;color:var(--text-muted);margin-bottom:12px;">${TBO_FORMATTER.escapeHtml(person.cargo)} \u2022 ${TBO_FORMATTER.escapeHtml(person.area)}</div>
      ...
      <div class="card" style="padding:20px;">
        ...
        <p style="font-size:0.85rem;color:var(--text-secondary);line-height:1.6;margin:0;">${TBO_FORMATTER.escapeHtml(review.parecer)}</p>
      </div>
    `;
  },
  ...
  // ══════════════════════════════════════════════════════════════════
  // AVALIACAO > AVALIAR (Real evaluation form)
  // ══════════════════════════════════════════════════════════════════
  _renderAvaliarForm(cycle, reviews) {
    if (!cycle || cycle.status === 'finalizado') return '<div class="empty-state" style="padding:32px;"><div class="empty-state-text">Ciclo finalizado. Inicie um novo ciclo para avaliar.</div></div>';
    ...
    const now = new Date();
    const currentPhase = cycle.fases.find(f => new Date(f.inicio) <= now && now <= new Date(f.fim)) || cycle.fases[0];
    const phaseId = currentPhase?.id || 'auto';

    // Who can be evaluated
    let targets = [];
    if (phaseId === 'auto') {
      // Self evaluation: only yourself
      targets = reviewable.filter(t => t.id === userId);
    } else if (phaseId === 'gestor') {
      // Manager evaluates direct reports
      if (isAdmin) targets = reviewable;
      else targets = reviewable.filter(t => t.lider === userId);
    } else if (phaseId === 'pares') {
      // Peer evaluation: evaluate peers (same leader)
      const me = this._getPerson(userId);
      if (isAdmin) targets = reviewable;
      else targets = reviewable.filter(t => t.id !== userId && t.lider === (me?.lider || ''));
    } else {
      if (isAdmin) targets = reviewable;
    }

    const sourceLabel = { auto: 'Autoavaliacao', gestor: 'Avaliacao do Gestor', pares: 'Avaliacao de Pares', calibracao: 'Calibracao' };

    return `
      <div class="card" style="margin-bottom:16px;">
        <div class="card-header">
          <div>
            <h3 class="card-title">Formulario de Avaliacao</h3>
            <div style="font-size:0.78rem;color:var(--text-muted);">Fase atual: <strong style="color:var(--color-info);">${sourceLabel[phaseId] || phaseId}</strong> — ${cycle.nome}</div>
          </div>
        </div>
        <div style="padding:16px;">
          <div class="form-group" style="margin-bottom:16px;max-width:400px;">
            <label class="form-label">Colaborador(a) a avaliar</label>
            <select class="form-input" id="avalTarget">
              <option value="">Selecione...</option>
              ${targets.map(t => `<option value="${t.id}">${t.nome} — ${t.cargo}</option>`).join('')}
            </select>
          </div>

          <div id="avalFormFields" style="display:none;">
            <div style="font-size:0.82rem;font-weight:600;margin-bottom:12px;color:var(--text-secondary);">Notas por Competencia (1 a 5)</div>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;margin-bottom:16px;">
              ${this._competenciasRadar.map(c => `
                <div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--bg-elevated);border-radius:var(--radius-md,8px);">
                  <span style="font-size:0.82rem;flex:1;font-weight:500;">${c.nome}</span>
                  <div style="display:flex;gap:4px;" data-comp="${c.id}">
                    ${[1,1.5,2,2.5,3,3.5,4,4.5,5].map(n => `
                      <button class="aval-score-btn" data-comp="${c.id}" data-score="${n}" style="width:${n % 1 === 0 ? '28px' : '22px'};height:28px;border:1px solid var(--border-default);border-radius:4px;background:var(--bg-primary);font-size:${n % 1 === 0 ? '0.72rem' : '0.62rem'};cursor:pointer;display:flex;align-items:center;justify-content:center;">${n}</button>
                    `).join('')}
                  </div>
                </div>
              `).join('')}
            ...
            <div class="grid-2" style="gap:16px;margin-bottom:16px;">
              <div class="form-group" style="margin-bottom:0;">
                <label class="form-label">Destaques (separados por virgula)</label>
                <input class="form-input" id="avalDestaques" placeholder="Ex: Lideranca, Qualidade Tecnica...">
              </div>
              <div class="form-group" style="margin-bottom:0;">
                <label class="form-label">Gaps de Desenvolvimento (separados por virgula)</label>
                <input class="form-input" id="avalGaps" placeholder="Ex: Comunicacao, Prazos...">
              </div>
            </div>

            <div class="form-group" style="margin-bottom:16px;">
              <label class="form-label">Parecer / Comentario</label>
              <textarea class="form-input" id="avalParecer" rows="3" placeholder="Parecer descritivo sobre o desempenho..."></textarea>
            </div>

            <button class="btn btn-primary" id="avalSubmit">Submeter Avaliacao</button>
          </div>
        </div>
      </div>

      <!-- Avaliacoes ja submetidas -->
      <div class="card">
        <div class="card-header"><h3 class="card-title">Avaliacoes Submetidas neste Ciclo</h3></div>
        <div style="max-height:300px;overflow-y:auto;">
          ${reviews.filter(r => r.cicloId === cycle.id).map(r => {
            const p = this._getPerson(r.pessoaId);
            return `<div style="padding:10px 16px;border-bottom:1px solid var(--border-subtle);display:flex;align-items:center;justify-content:space-between;">
              <div><strong style="font-size:0.85rem;">${p ? p.nome : r.pessoaId}</strong> <span style="font-size:0.72rem;color:var(--text-muted);">${p ? p.cargo : ''}</span></div>
              <span style="font-size:0.9rem;font-weight:700;color:${r.mediaGeral >= 4 ? 'var(--color-success)' : r.mediaGeral >= 3 ? 'var(--accent-gold)' : 'var(--color-danger)'};">${r.mediaGeral.toFixed(1)}</span>
            </div>`;
          }).join('') || '<div style="padding:16px;font-size:0.78rem;color:var(--text-muted);">Nenhuma avaliacao submetida.</div>'}
        </div>
      </div>
    `;
  },
  ...
  _submitAvaliacao(cycleId) {
    ...
    const phaseMap = { auto: 'autoScores', gestor: 'gestorScores', pares: 'paresScores' };
    const ciclos = this._getStore('ciclos');
    const cycle = ciclos.find(c => c.id === cycleId);
    const now = new Date();
    const currentPhase = cycle?.fases.find(f => new Date(f.inicio) <= now && now <= new Date(f.fim)) || cycle?.fases[0];
    const phaseId = currentPhase?.id || 'auto';
    const scoreField = phaseMap[phaseId] || 'autoScores';

    // Collect scores — validate that at least one competency was explicitly rated
    const scores = this._competenciasRadar.map(c => {
      const activeBtn = document.querySelector(`.aval-score-btn[data-comp="${c.id}"].active`);
      return { comp: c.id, nota: activeBtn ? parseFloat(activeBtn.dataset.score) : null, explicit: !!activeBtn };
    });
    const hasAnyExplicit = scores.some(s => s.explicit);
    if (!hasAnyExplicit) {
      TBO_TOAST.warning('Avalie pelo menos uma competencia antes de submeter.');
      return;
    }
    // Fill unrated competencies with default 3.0
    scores.forEach(s => { if (s.nota == null) s.nota = 3; delete s.explicit; });

    const destaques = (document.getElementById('avalDestaques')?.value || '').split(',').map(s => s.trim()).filter(Boolean);
    const gaps = (document.getElementById('avalGaps')?.value || '').split(',').map(s => s.trim()).filter(Boolean);
    ...
    if (review) {
      // Update existing review's scores for this phase
      review[scoreField] = scores;
      if (destaques.length) review.destaques = destaques;
      if (gaps.length) review.gaps = gaps;
      if (parecer) review.parecer = parecer;
    } else {
      // Create new review — default scores = 3.0 for phases not yet evaluated
      const defaultScores = this._competenciasRadar.map(c => ({ comp: c.id, nota: 3 }));
      review = {
        id: this._genId(), cicloId: cycleId, pessoaId: targetId,
        autoScores: defaultScores.map(s => ({ ...s })),
        gestorScores: defaultScores.map(s => ({ ...s })),
        paresScores: defaultScores.map(s => ({ ...s })),
        destaques, gaps, parecer
      };
      review[scoreField] = scores;
      reviews.push(review);
    }
    ...
    review.autoMedia = avg(review.autoScores);
    review.gestorMedia = avg(review.gestorScores);
    review.paresMedia = avg(review.paresScores);
  ...
  _renderNineBox(reviews) {
    ...
    // Desempenho = gestorMedia, Potencial = media de criatividade+aprendizado+produtividade (scores do gestor)
    const peopleData = cycleReviews.map(r => {
      const person = this._getPerson(r.pessoaId);
      const desemp = r.gestorMedia;
      const potentialComps = ['criatividade', 'aprendizado', 'produtividade'];
      ...
      return `
        <div style="padding:14px 16px;border-bottom:1px solid var(--border-subtle);display:flex;gap:12px;align-items:flex-start;">
          ...
          <div style="flex:1;">
            ...
            <div style="font-size:0.82rem;color:var(--text-secondary);line-height:1.5;">${TBO_FORMATTER.escapeHtml(f.mensagem)}</div>
    ...
    return elogios.sort((a, b) => new Date(b.data) - new Date(a.data)).map(e => {
      ...
      return `
        <div style="padding:16px;border-bottom:1px solid var(--border-subtle);">
          ...
          <div style="font-size:0.85rem;color:var(--text-secondary);line-height:1.6;padding-left:42px;">${TBO_FORMATTER.escapeHtml(e.mensagem)}</div>
  ...
  init() {
    ...
    if (avalTarget) {
      avalTarget.addEventListener('change', () => {
        ...
        if (avalTarget.value) {
          ...
          const activeCycle = ciclos.find(c => c.status === 'em_andamento') || ciclos[0];
          const reviews = this._getStore('avaliacoes_people');
          const existing = reviews.find(r => r.pessoaId === avalTarget.value && r.cicloId === activeCycle?.id);
          if (existing) {
            // Determine phase
            const now = new Date();
            const phase = activeCycle?.fases.find(f => new Date(f.inicio) <= now && now <= new Date(f.fim)) || activeCycle?.fases[0];
            const scoreField = { auto: 'autoScores', gestor: 'gestorScores', pares: 'paresScores' }[phase?.id] || 'autoScores';
            (existing[scoreField] || []).forEach(s => {
              const btn = document.querySelector(`.aval-score-btn[data-comp="${s.comp}"][data-score="${s.nota}"]`);
              if (btn) { btn.classList.add('active'); btn.style.background = 'var(--accent-gold)'; btn.style.color = '#fff'; }
            });
            if (existing.destaques?.length) document.getElementById('avalDestaques').value = existing.destaques.join(', ');
            if (existing.gaps?.length) document.getElementById('avalGaps').value = existing.gaps.join(', ');
            if (existing.parecer) document.getElementById('avalParecer').value = existing.parecer;
          }
        }
      });
    }
    ...
    document.querySelectorAll('.aval-score-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll(`.aval-score-btn[data-comp="${comp}"]`).forEach(b => {
    ...
    this._bind('avalSubmit', () => {
      const ciclos = this._getStore('ciclos');
      const activeCycle = ciclos.find(c => c.status === 'em_andamento') || ciclos[0];
      if (activeCycle) this._submitAvaliacao(activeCycle.id);
    });
